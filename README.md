...
│   └── checkoff_repository.py   # Check-off data access layer
│
├── models/
│   ├── habit.py                 # Habit class definition (OOP)
│   └── checkoff.py              # Check-off model
│
├── services/
...
**Check-off Model** (`models/checkoff.py`)

Represents a single completion event: 
- `habit_name`: Associated habit
...
- **Models**:  Data structures (Habit, Checkoff)
...
calculate_current_streak(habit_name)
```
...
- **Schema:**
  ...
  - `checkoffs` table: Tracks completion timestamps
- **Operations:**
